...
class GameCategoryTests(APITestCase):
    ...
    def test_create_duplicated_game_category(self):
        ...
        duplicated_response = self.create_game_category(new_game_category_name)
    ...
    def test_update_game_category(self):
        ...
        update_game_category_name = 'Updated Game Category Name'
        data = {'name': update_game_category_name}
        patch_response = self.client.patch(url, data, format='json')
        self.assertEqual(patch_response.status_code, status.HTTP_200_OK)
        self.assertEqual(patch_response.data['name'], update_game_category_name)
